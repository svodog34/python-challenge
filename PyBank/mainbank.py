...
#Default variables for what we are looking for
total_months = 0
total_revenue = 0
prev_revenue = 0
month_of_change = []
revenue_change_list = []
biggest_dec = ['', 99999999999]
biggest_inc = ['', 0]
...
with open(pybank_csv_path,newline="") as csvfile:  
    ...
    for row in reader:
        ...
        if rev_change>biggest_inc[1]:
            biggest_inc[1]= rev_change
            biggest_inc[0] = row['Date']
        #The greatest decrease in revenue (date and amount) over the entire period
        if rev_change<biggest_dec[1]:
            biggest_dec[1]= rev_change
            biggest_dec[0] = row['Date']
